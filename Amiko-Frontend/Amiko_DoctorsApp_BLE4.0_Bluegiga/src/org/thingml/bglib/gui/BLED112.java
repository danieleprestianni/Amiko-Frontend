...
public class BLED112 {
	...
	public static String selectSerialPort() {
		...
		return (String) JOptionPane.showInputDialog(null, "BLED112", "Select serial port", JOptionPane.PLAIN_MESSAGE,
				null, possibilities.toArray(), possibilities.toArray()[startPosition]);

	}

	/**
	 * AMIKO TODO verificare vincoli licenza del codice sorgente
	 * 
	 * @return
	 * @throws IOException
	 * @throws SecurityException
	 */
	public static String autoselectSerialPort(int vid, int pid) throws SecurityException, IOException {
		...
		try {
			...
			for (CommPortIdentifier commportidentifier : getAvailableSerialPorts()) {
				// check if commportidentifier.getName() is associated to
				// Bluegiga BLED112 device
				String[] serialPorts = scm.findComPortFromUSBAttributes(vid, pid, null);
				if (serialPorts != null && serialPorts.length > 0
						&& serialPorts[0].contains(commportidentifier.getName())) {
					possibilities.add(commportidentifier.getName());
				}
			}
		...
		if (possibilities.size() == 0) {
			...
			return (String) JOptionPane.showInputDialog(null, "BLED112", "Select serial port",
					JOptionPane.PLAIN_MESSAGE, null, possibilities.toArray(), possibilities.toArray()[startPosition]);
		}
	}
	...
	static {
		...
		try {
			// This is a very dirty hack to try and set the java.library.path
			// dynamically.
		...
		try {
			...
			System.out.println("Load RxTx for os.name=" + osName + " os.arch=" + osProc + " sun.arch.data.model="
					+ System.getProperty("sun.arch.data.model"));
			System.out.println("Current path = " + new File(".").getAbsolutePath());

			if (osName.equals("Mac OS X")) {
				System.out.println("Copiying " + "nativelib/Mac_OS_X/librxtxSerial.jnilib");
				NativeLibUtil.copyFile(NativeLibUtil.class.getClassLoader()
						.getResourceAsStream("nativelib/Mac_OS_X/librxtxSerial.jnilib"), "librxtxSerial.jnilib");
			}
			if (osName.contains("Win") && System.getProperty("sun.arch.data.model").contains("32")) {
				...
				NativeLibUtil.copyFile(NativeLibUtil.class.getClassLoader()
						.getResourceAsStream("nativelib/Windows/win32/rxtxSerial.dll"), "rxtxSerial.dll");
			}
			if (osName.contains("Win") && System.getProperty("sun.arch.data.model").contains("64")) {
				...
				NativeLibUtil.copyFile(NativeLibUtil.class.getClassLoader()
						.getResourceAsStream("nativelib/Windows/win64/rxtxSerial.dll"), "rxtxSerial.dll");
			}
			if (osName.equals("Linux") && osProc.equals("ia64")) {
				...
				NativeLibUtil.copyFile(NativeLibUtil.class.getClassLoader().getResourceAsStream(
						"nativelib/Linux/ia64-unknown-linux-gnu/librxtxSerial.so"), "librxtxSerial.so");
				...
				NativeLibUtil
						.copyFile(
								NativeLibUtil.class.getClassLoader().getResourceAsStream(
			...
			if (osName.equals("Linux") && System.getProperty("sun.arch.data.model").contains("32")) {
				...
				NativeLibUtil
						.copyFile(
								...
								"librxtxParallel.so");
				NativeLibUtil.copyFile(NativeLibUtil.class.getClassLoader().getResourceAsStream(
						"nativelib/Linux/i686-unknown-linux-gnu/librxtxSerial.so"), "librxtxSerial.so");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
