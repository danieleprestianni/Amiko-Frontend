...
import io.amiko.app.utils.export.DataTable;
...
public class MainController implements BLEDriverListener {
	...
	private boolean devModeIsEnabled = true;
	...
	public void onBLEDeviceConnected(BLEDevice device) {
		if (device != null && this.currentDevice != null) {
			...
			this.startBatteryCheckTimer();
			this.startTimerCheckTimer();

			// get the Amiko device Serial Code
			if (this.currentDevice != null) {
				logger.debug("[SEND-REQUEST]-Send command: GET_SERIAL_CODE");
				AmikoCmd cmd = AmikoCmdFactory.makeGetSerialCodeCmd();
	...
	public void onResponse(AmikoCmdResp response) {
		...
		if (response.getResponseType() == ResponseType.GET_BATTERY_CMD_RESP) {
			...
			// A = (batteryLevel+730)*3.5/1000  = tensione in mV variabile tra 0..3
			...
			Display.getDefault().asyncExec(new Runnable() {
				@Override
				public void run() {
					MainController.this.mainWindow.updateBatteryInfo(_batteryLevel);
				}
			});
			...
			Display.getDefault().asyncExec(new Runnable() {
				@Override
				public void run() {
					MainController.this.mainWindow.updateCurrentTimeInfo(formatter.format(d));
				}
			});
			...
			Display.getDefault().asyncExec(new Runnable() {
				@Override
				public void run() {
					MainController.this.mainWindow.enableCleanButton(true);
				}
			});
			...
			if(totalRecordsNumber>=this.totalRecordsNumber){
				// flash memory contains the same or more records respect the previous session 
				int delta = (totalRecordsNumber-this.totalRecordsNumber);
				this.recordsNumber += delta;
				logger.debug("[RECOVERY-RECS-DOWNLOAD]-Device contains the same or more records respect the previous session DELTA=" + delta + " TOTAL=" + totalRecordsNumber);
				...
				// flash meory contains less records - we have to interrupt the download auto recovery
			...
			if (this.recordsNumber > 0) {
				...
				// update the UI
				if(this.devModeIsEnabled){
					Display.getDefault().asyncExec(new Runnable() {
						@Override
						public void run() {
							int loaderValue = ((totalRecordsNumber - recordsNumber) * 100 / totalRecordsNumber);
							MainController.this.mainWindow.insertTableRow(rec, loaderValue);
						}
					});					
				}else{
					Display.getDefault().asyncExec(new Runnable() {
			...
			try {
				...
				Display.getDefault().asyncExec(new Runnable() {
					@Override
					public void run() {
						MainController.this.mainWindow.updateSerialCode(serialCodeStr);
					}
				});
	...
	public void onEventConnectBtnClicked() {
		...
		if (this.currentDevice != null && this.connectionIsEnabled) {
			...
			// update the UI
			this.mainWindow.resetBatteryInfo();
			this.mainWindow.resetCurrentTimeInfo();
		}
	}
	...
	public void onEventCleanBtnClicked() {
		...
		if (this.currentDevice != null) {
			...
			this.mainWindow.enableCleanButton(false);
	...
	public void onEventExportBtnCicked() {
		// #EXPORT
		DataTable<RecordModel> exportData = new DataTable<RecordModel>();
		if (this.tableData != null && this.tableData.size() > 0) {
			// populate the DataTable with all available RecordModel
			for (int i = 0; i < this.tableData.size(); i++) {
				RecordModel rec = this.tableData.get(i);
				if(rec!=null && rec.getIndex()>=0){
					exportData.addRow(rec);
				}
			}
			...
			// get the next export file name
			String filename = this.deviceName + "_data_" + dateStr + ".csv";
			ArrayList<String> tableFields = new ArrayList<String>();
			// NOTA: please pay attention to the first character upper
			// case
			tableFields.add("Index");
			tableFields.add("Timestamp");
			tableFields.add("EventId");
			tableFields.add("Data0");
			tableFields.add("Data1");
			tableFields.add("Data2");
			tableFields.add("Data3");
			tableFields.add("Data4");
			tableFields.add("Data5");
			exportData.exportAsCSV(new File(filename), tableFields);

			this.mainWindow.setStatusMessage("Data exported: " + filename, false);

			logger.debug("Records have been exported: " + filename);
		}
	}
	...
	private void startBatteryCheckTimer() {
		this.batteryCheckerTimer.scheduleAtFixedRate(new TimerTask() {
			@Override
			public void run() {
				if (MainController.this.currentDevice != null) {
					logger.debug("[SEND-REQUEST]-Send command: GET_BATTERY");
					AmikoCmd cmd = AmikoCmdFactory.makeGetBatteryCmd();
					MainController.this.driver.sendCmd(cmd);
				} else {
					// no available connected device

					// update the UI
					Display.getDefault().asyncExec(new Runnable() {
						@Override
						public void run() {
							MainController.this.mainWindow.resetBatteryInfo();
						}
					});
	...
	private void startTimerCheckTimer() {
		this.timerCheckerTimer.scheduleAtFixedRate(new TimerTask() {
			@Override
			public void run() {
				if (MainController.this.currentDevice != null) {
					logger.debug("[SEND-REQUEST]-Send command: GET_TIME");
					AmikoCmd cmd = AmikoCmdFactory.makeGetTimeCmd();
					MainController.this.driver.sendCmd(cmd);
				} else {
					// no available connected device
					...
					Display.getDefault().asyncExec(new Runnable() {
						@Override
						public void run() {
							MainController.this.mainWindow.resetCurrentTimeInfo();
						}
					});
				}
			}
